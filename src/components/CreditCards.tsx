  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <p className="text-sm text-gray-600 dark:text-gray-400">Total Weekly Payment</p>
      ...
      <div className="space-y-4">
        {cards.length === 0 ? (
          ...
          cards.map((card) => {
            ...
            return (
              <div key={card.id} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h2 className="text-xl font-semibold dark:text-white">{card.name}</h2>
                ...
                {card.plans.length === 0 ? (
                  <p className="text-gray-500 dark:text-gray-400 text-sm">No plans added</p>
                ) : (
                  <>
                    {/* Active Plans */}
                                ...
                                <div className="flex-1">
                                  ...
                                  {payments.length > 0 && (
                                    <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                                      <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Payment History ({payments.length}):</p>
                                      <div className="space-y-1 max-h-32 overflow-y-auto">
                                        {payments
                                          .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                                          .map((payment) => (
                                            <div key={payment.id} className="flex justify-between items-center text-xs group hover:bg-gray-50 dark:hover:bg-gray-700 px-1 py-0.5 rounded">
                                              <span className="text-gray-600 dark:text-gray-400">{formatDate(payment.date)}</span>
                    ...
                    {card.plans.filter((plan) => {
                      ...
                      <div className="mt-4">
                        <button
                          ...
                          className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 w-full"
                        >
                          ...
                          <div className="space-y-3 mt-3">
                            {card.plans
                              ...
                              .map((plan) => {
                                const payments = plan.payments || [];
                                return (
                                  <div key={plan.id} className="border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 rounded-lg p-4 opacity-75">
                                    <div className="flex justify-between items-start">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                          <h3 className="font-semibold text-green-800 dark:text-green-400">{plan.name}</h3>
                                          <span className="text-xs bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-300 px-2 py-1 rounded-full font-semibold">
                                            PAID OFF
                                          </span>
                                        </div>
                                          ...
                                          <div className="mt-3 pt-3 border-t border-green-200 dark:border-green-800">
                                            ...
                                            <div className="space-y-1 max-h-32 overflow-y-auto">
                                              {payments
                                                .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                                                .map((payment) => (
                                                  <div key={payment.id} className="flex justify-between items-center text-xs group hover:bg-green-100 dark:hover:bg-green-900/20 px-1 py-0.5 rounded">
                                                    <span className="text-gray-600 dark:text-gray-400">{formatDate(payment.date)}</span>
                                                    <div className="flex items-center gap-2">
                                                      <span className="font-semibold text-green-600">{formatCurrency(payment.amount)}</span>
                                                      <button
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h2 className="text-2xl font-bold dark:text-white mb-4">Add Credit Card</h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleAddPlan} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Interest-Free Months</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Interest-Free End Date</label>
                <input
                  type="date"
      ...
      {showPaymentModal && selectedCardId && selectedPlanId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h2 className="text-2xl font-bold dark:text-white mb-4">Make Payment</h2>
